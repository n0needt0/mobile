...
class SimpleTester
{
  ...
  */
  ...
  public function run_tests($test_file=false)
  {
    echo $this->Run($test_file);
    die;
  }
    ...
    if(is_dir($this->testDir))
    {
      if($dh = opendir($this->testDir))
      {
        while(($file = readdir($dh)) !== FALSE)
        {
          ...
          if( (substr($file, -(strlen($this->fileExtension)+1)) == '.' . $this->fileExtension) && (substr($file, 0,8)) != 'TEMPLATE' )
          {
            if(($test_file !== false) && ($file != $test_file . '.'.$this->fileExtension))
            {
              continue;
            }

            ob_start();
            $filebase = str_replace('.' . $this->fileExtension,'',$file);

            if(stripos(current_url(),$filebase) === false)
            {
              $link = anchor(current_url() . '/' . $filebase, $file);
            }
             else
            {
              $link = anchor(str_replace($filebase,'',current_url()) , $file);
            }

            $group_test = new GroupTest('test in: ' . $link);
            $group_test->addTestFile($this->testDir . "/$file");
