...
class SimpleTestCase {
  ...
  function _coverage($test=false,$target=false)
  {
    if(!$target || !$test)
    {
      return;
    ...
    $target_methods = $target_reflection->getMethods();
    $test_methods = $test_reflection->getMethods();
    ...
    {
      if(($method->class == $test_methods[0]->class) && (substr($method->name,0,2) != '__') )
      {
        //we only care about top level methods i.e. not parent ones
        //we also dont care for magic methods
        $tests[] = $method->name;
      }
    }

    foreach($target_methods as $key=>$method)
    {
      if(($method->class == $target_methods[0]->class) && (substr($method->name,0,2) != '__') )
      {
        //we only care about top level methods i.e. not parent ones
        //we also dont care for magic methods
        $missing_test = 'test_' . $method->name;
        if(!in_array($missing_test,$tests))
        {
          $notcovered++;
          if(isset($_REQUEST['detailed']))
          {
            echo 'TODO ' . $missing_test . '<BR/>';
          }
        }
         else
        {
    ...
    $codecoverage_label = "Test Code Coverage = " . number_format(100 * $covered/($covered + $notcovered),0) . ' % ';

    if(isset($_REQUEST['detailed']))
    {

      echo $codecoverage_label . anchor(str_replace('?detailed','',current_url()), 'hide details');
    }
     else
    {
      echo $codecoverage_label . anchor(str_replace('?detailed','',current_url()) . "?detailed", 'detailed');
    }
  }
